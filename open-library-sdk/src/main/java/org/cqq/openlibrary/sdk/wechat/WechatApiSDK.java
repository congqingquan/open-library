...
import org.cqq.openlibrary.sdk.wechat.request.base.WechatGetRequest;
import org.cqq.openlibrary.sdk.wechat.request.base.WechatPostRequest;
import org.cqq.openlibrary.sdk.wechat.request.base.WechatRequest;
...
public class WechatApiSDK {
    ...
    public static void checkResponse(String apiDesc, WechatResponse<?> response) {
        if (response.getErrCode() != 0) {
            ...
            throw new WechatSDKException(apiDesc + " response error");
        }
    }
    ...
    public static <Response extends WechatResponse<Response>, Request extends WechatRequest<Response>> Response execute(Request request) {
        ...
        if (request instanceof WechatGetRequest<?> getRequest) {
            ...
            switch (apiEnum.getMediaType()) {
                ...
                // case ...
                default -> throw new WechatSDKException("暂不支持的 Post media type");
            }
        } else {
            throw new WechatSDKException("暂不支持的 HTTP 请求方式");
        }
